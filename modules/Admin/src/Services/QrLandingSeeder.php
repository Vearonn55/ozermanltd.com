...
/**
 * Creates/updates CMS pages for /qr and /catalogues (editable embeds).
 */
class QrLandingSeeder
{
    public function __construct(
        private readonly PageRepository $repo,
    ...
    public function run(): array
    {
        ...
        foreach ($this->definitions() as $def) {
            $existing = $this->pdo->prepare(
                'SELECT id FROM pages WHERE slug = :slug OR custom_path = :path LIMIT 1'
            );
            ...
            if ($id) {
                $this->repo->update((int) $id, $payload);
                $messages[] = "Updated /{$def['custom_path']} (page #{$id})";
            } else {
                $newId = $this->repo->create($payload);
                $messages[] = "Created /{$def['custom_path']} (page #{$newId})";
            }
        }
    ...
    {
        $missing = [];
        foreach (['qr', 'catalogues'] as $slug) {
            $stmt = $this->pdo->prepare(
                'SELECT id FROM pages WHERE slug = :slug OR custom_path = :slug LIMIT 1'
            );
    ...
    /** @return list<array{slug: string, custom_path: string, title: string, html: string, css: string}> */
