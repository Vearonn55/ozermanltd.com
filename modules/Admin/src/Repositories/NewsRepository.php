...
class NewsRepository extends BaseAdminRepository
{
    ...
    public function paginate(string $q = '', string $status = '', int $page = 1, int $perPage = 15): array
    {
        ...
        $stmt = $this->pdo->prepare(
            "SELECT n.*, nt.title, nt.slug, nct.name AS category_name {$base}
             ORDER BY n.publish_date DESC, n.id DESC
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO news (category_id, status, publish_date, is_featured, is_pinned, featured_image_id, author_id)
             VALUES (:category_id, :status, :publish_date, :is_featured, :is_pinned, :featured_image_id, :author_id)'
        );
        $stmt->execute([
    ...
    public function update(int $id, array $data): void
    {
        $stmt = $this->pdo->prepare(
            'UPDATE news SET category_id = :category_id, status = :status, publish_date = :publish_date,
             is_featured = :is_featured, is_pinned = :is_pinned, featured_image_id = :featured_image_id
             WHERE id = :id'
        );
        $stmt->execute([
            'id' => $id,
