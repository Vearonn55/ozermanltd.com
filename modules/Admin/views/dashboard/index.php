...
<div class="bg-white rounded-xl border border-slate-200 p-5 mb-8">
    <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-3">Quick actions</h2>
    <div class="flex flex-wrap gap-2">
        <?php
        $actions = [
            ['label' => 'New article', 'href' => 'news/create'],
            ['label' => 'New project', 'href' => 'projects/create'],
            ['label' => 'Upload media', 'href' => 'media'],
            ['label' => 'Edit menus', 'href' => 'menus'],
            ['label' => 'Site sections', 'href' => 'site-sections'],
            ['label' => 'Settings', 'href' => 'settings'],
        ];
        foreach ($actions as $action):
        ?>
        <a href="<?= admin_url($action['href']) ?>"
           class="inline-flex items-center rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-700 transition-colors">
            <?= e($action['label']) ?>
        </a>
        <?php endforeach; ?>
    </div>
        ...
        <ul class="divide-y divide-slate-100">
            ...
            <li class="py-3 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <?php $status = $contact['status']; require ADMIN_MODULE_PATH . '/views/partials/status-badge.php'; ?>
...
<div class="bg-white rounded-xl border border-slate-200 p-6">
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold">Recent drafts</h2>
        <a href="<?= admin_url('news?status=draft') ?>" class="text-sm text-blue-600 hover:underline">View news drafts</a>
    </div>
    <?php if (empty($drafts)): ?>
    <p class="text-sm text-slate-500">No drafts right now. Published content is ready for the site.</p>
    <?php else: ?>
    <ul class="divide-y divide-slate-100">
        <?php foreach ($drafts as $draft): ?>
        <li class="py-3 flex items-center justify-between gap-3">
            <div class="min-w-0">
                <p class="text-sm font-medium text-slate-900 truncate"><?= e($draft['title']) ?></p>
                <p class="text-xs text-slate-500"><?= e($draft['type']) ?><?php if (!empty($draft['updated'])): ?> · <?= e(date('M j, Y', strtotime((string) $draft['updated']))) ?><?php endif; ?></p>
            </div>
            <a href="<?= admin_url($draft['href']) ?>" class="text-sm text-blue-600 hover:underline shrink-0">Edit</a>
        </li>
        <?php endforeach; ?>
    </ul>
    <?php endif; ?>
