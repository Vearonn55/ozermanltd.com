...
<form method="POST" action="<?= $isEdit ? admin_url('projects/' . $item['id']) : admin_url('projects') ?>" class="space-y-6 max-w-4xl"
      <?= $isEdit ? 'data-autosave-url="' . admin_url('projects/' . $item['id'] . '/autosave') . '"' : '' ?>>
    <?= csrf_field() ?>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        <div class="flex items-center justify-between">
            ...
            <a href="<?= e(admin_preview_url('projects/' . $viewSlug)) ?>" target="_blank" class="text-sm text-blue-600 hover:underline">View on site ↗</a>
            <?php endif; ?>
        </div>
    ...
    $fields = [
        ...
        'features' => ['type' => 'textarea', 'label' => 'Features (one per line, prefix with •)'],
        'payment_plan' => 'textarea',
        'meta_title' => 'text',
...
<div class="max-w-4xl mt-6">
    <?php admin_partial('revisions', [
        ...
        'editUrl' => admin_url('projects/' . $item['id'] . '/edit'),
    ]); ?>
</div>
<?php endif; ?>
