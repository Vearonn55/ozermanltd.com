<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-slate-500"><?= count($items) ?> store(s)</p>
    <a href="<?= admin_url('stores/create') ?>" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg">+ New Store</a>
</div>

<div x-data="adminBulk()">
    <form id="bulk-form" method="POST" action="<?= admin_url('stores/bulk') ?>"
          onsubmit="return this.elements.bulk_action.value !== 'delete' || confirm('Delete the selected stores?')"
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-4 py-3">
                        <a href="<?= admin_url('stores/' . $item['id'] . '/edit') ?>" class="font-medium hover:text-blue-600"><?= e($item['label'] ?? 'Untitled') ?></a>
                    </td>
                    ...
                    <td class="px-4 py-3 text-right whitespace-nowrap space-x-2">
                        <form method="POST" action="<?= admin_url('stores/' . $item['id'] . '/status') ?>" class="inline">
                            <?= csrf_field() ?>
                            <button type="submit" class="text-slate-600 hover:underline"><?= $item['is_active'] ? 'Hide' : 'Show' ?></button>
                        </form>
                        <a href="<?= admin_url('stores/' . $item['id'] . '/edit') ?>" class="text-blue-600 hover:underline">Edit</a>
                        <form method="POST" action="<?= admin_url('stores/' . $item['id']) ?>" class="inline" onsubmit="return confirm('Delete this store?')">
                            <?= csrf_field() ?>
                ...
                <?php endforeach; ?>
                <?php if (!$items): ?>
                <tr><td colspan="6" class="px-4 py-8 text-center text-slate-500">No stores yet. Create the first one.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
